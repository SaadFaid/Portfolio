...
const projects: Project[] = [
  { number: '01', title: 'Bghit Nsog', name: 'Car rental mobile app', description: 'A mobile-first rental flow for discovering cars, making reservations, and keeping rental operations moving.', tags: ['Flutter', 'Spring Boot', 'REST APIs'], category: 'selected', art: 'project-art--car', image: '/images/bghit-nsog.png', bg: '#ff4d14', href: 'https://github.com/SaadFaid/BghitNsog_RentalCar_MobileApplication' },
  ...
  { number: '03', title: 'The fleet, in one place', name: 'Rental agency management platform', description: 'Web surfaces for agencies to coordinate vehicles, reservations, announcements, and fleet availability.', tags: ['React.js', 'Node.js', 'MongoDB'], category: 'selected', art: 'project-art--fleet', image: '/images/rental-car.png', bg: '#2e2e2e', href: 'https://github.com/SaadFaid/VemoCar_RentalAgency' },
...
const skills: Skill[] = [
  ...
  { name: 'MariaDB', group: 'Data', icon: SiMysql },
  ...
  return (
    <section id="work" className="mx-auto max-w-[1440px] px-6 py-24 lg:px-10 lg:py-36">
      ...
      <div className="reveal reveal-delay-2 mt-8 grid items-start gap-5 lg:grid-cols-[1.1fr_.9fr]">
        {activeProject && <div id="work-spotlight" className="grid-panel relative min-h-[480px] scroll-mt-28 overflow-hidden border border-foreground/20 p-3"><ProjectVisual art={activeProject.art} image={activeProject.image} bg={activeProject.bg} /><div className="absolute inset-x-7 bottom-7 z-10 flex flex-wrap items-end justify-between gap-x-4 gap-y-3"><div className="min-w-0"><p className="font-mono-ui text-[10px] uppercase tracking-[.13em] text-[hsl(var(--accent))]">Spotlight / {activeProject.number}</p><h3 className="mt-3 max-w-md font-display text-2xl font-bold uppercase leading-[.95] text-foreground sm:text-3xl">{activeProject.title}</h3></div><div className="flex shrink-0 flex-wrap items-center gap-2">{activeProject.href && <a href={activeProject.href} target="_blank" rel="noreferrer" data-testid={`link-spotlight-${activeProject.number}`} className="inline-flex items-center gap-2 border border-foreground/30 bg-[hsl(var(--background)/.75)] px-3 py-2 font-mono-ui text-[9px] uppercase tracking-[.12em] text-foreground/70 transition-colors hover:border-[hsl(var(--accent))] hover:text-foreground"><Github className="h-4 w-4" /> View on GitHub <ArrowUpRight className="h-3.5 w-3.5" /></a>}<button type="button" aria-label="Previous project" data-testid="button-project-previous" onClick={() => moveProject(-1)} className="grid h-10 w-10 place-items-center border border-foreground/30 bg-[hsl(var(--background)/.75)] transition-colors hover:border-[hsl(var(--accent))]"><ArrowLeft className="h-4 w-4" /></button><button type="button" aria-label="Next project" data-testid="button-project-next" onClick={() => moveProject(1)} className="grid h-10 w-10 place-items-center border border-foreground/30 bg-[hsl(var(--background)/.75)] transition-colors hover:border-[hsl(var(--accent))]"><ArrowRight className="h-4 w-4" /></button></div></div></div>}
...
function Toolkit() {
  ...
  return (
    <section id="stack" className="border-y border-foreground/20 bg-white text-[hsl(var(--background))]">
      <div className="mx-auto max-w-[1440px] px-6 py-24 lg:px-10 lg:py-36">
        <div className="reveal flex flex-col justify-between gap-8 border-b border-[hsl(var(--background)/.22)] pb-10 lg:flex-row lg:items-end"><div><p className="eyebrow font-mono-ui text-[hsl(var(--background))]">04 / Toolkit</p><h2 className="mt-5 max-w-3xl font-display text-5xl font-bold uppercase leading-[.87] sm:text-7xl">The stack<br />behind the work.</h2></div><p className="max-w-sm text-sm leading-6 text-[hsl(var(--background)/.58)]">Recognizable tools, chosen for the problem in front of me. From structured data to the last interaction.</p></div>
        <div className="reveal reveal-delay-1 mt-10 grid grid-cols-2 gap-px border border-[hsl(var(--background)/.22)] bg-[hsl(var(--background)/.22)] sm:grid-cols-3 lg:grid-cols-5">{groups.map((group) => <div key={group} className="bg-[hsl(0_0%_78%)] p-4 sm:p-5"><p className="font-mono-ui text-[9px] uppercase tracking-[.15em] text-[hsl(var(--background))]">{group}</p><div className="mt-4 grid gap-2">{skills.filter((skill) => skill.group === group).map((skill) => { const Icon = skill.icon; return <div key={skill.name} className="skill-tile flex items-center gap-2 border border-[hsl(var(--background)/.16)] px-2.5 py-3 text-[hsl(var(--background)/.84)]" title={skill.name}>{Icon ? <Icon className="h-4 w-4 shrink-0" aria-hidden="true" /> : <span className="grid h-4 w-4 shrink-0 place-items-center border border-current font-mono-ui text-[7px]" aria-hidden="true">R</span>}<span className="font-mono-ui text-[9px] leading-3">{skill.name}</span></div>; })}</div></div>)}</div>
      </div>
    </section>
  );
}
...
function Footer() {
  return <footer className="bg-[hsl(var(--accent))] px-6 pb-8 text-[hsl(var(--accent-foreground)/.56)] lg:px-10"><div className="mx-auto flex max-w-[1440px] flex-col items-center justify-between gap-4 border-t border-[hsl(var(--accent-foreground)/.25)] pt-5 text-center font-mono-ui text-[9px] uppercase tracking-[.12em] sm:flex-row sm:text-left"><span className="flex flex-col items-center gap-1 sm:flex-row sm:items-center sm:gap-3"><span className="text-[hsl(var(--accent-foreground)/.9)]">© {new Date().getFullYear()} Saad Faid</span><span className="hidden text-[hsl(var(--accent-foreground)/.45)] sm:inline">/</span><span className="text-[hsl(var(--accent-foreground)/.9)]">Built by Saad Faid</span></span><span className="flex flex-wrap items-center justify-center gap-3 text-[hsl(var(--accent-foreground)/.9)]"><span data-testid="badge-available" className="flex items-center gap-2"><i className="h-1.5 w-1.5 rounded-full bg-[hsl(var(--accent-foreground))]" /> Available for work</span><span className="text-[hsl(var(--accent-foreground)/.5)]">/</span><span data-testid="badge-version">v2.0 — live</span></span></div></footer>;
}
